#! /usr/bin/env python
#-*- coding:utf-8 -*-
# _gen_sxhy_dict  生成诗学含英dict
# Segmenter      加载jieba自定义词典（sxhy_dict）
#  Segmenter.segment  利用sxhy 词典对sentence 进行分词
                ...
                if len(toks) == 3:
                    ...
                    for tok in toks:
                        if len(tok) < 4:
                            ...
                            tok_list.extend(jieba.lcut(tok, HMM=True, cut_all=True)) #全模式，把可能存在的中文词汇全部分出来，可能存在冗余
                    for tok in tok_list:
                        sxhy_dict[tok] = tag
            line = fin.readline().strip()
...
class Segmenter:
    ...
    def __init__(self):
        ...
        jieba.load_userdict(sxhy_path) #加载自定义词库

        self._sxhy_dict = get_sxhy_dict() #加载dict

    def segment(self, sentence):   #利用sxhy  sentences
        if 0 == len(sentence):
            return []
        elif 1 == len(sentence):
            return [sentence]
