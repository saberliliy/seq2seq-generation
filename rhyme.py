...
class RhymeUtil:
    ...
    def get_possible_tones(self, ch):   #返回汉字可能的声调，类似于[1,2]
        """
        Args:
            ch: A unicode character
    ...
    def get_possible_rhyme_categories(self, ch):
        ...
        filtered_categories = list(filter(None, rhyme_categories))
...
class RhymeEvaluator:
    ...
    def eval(self, sentences, output_all_scores=False, output_split=False):
        ...
        scores = list(map(lambda rule: self.score(rule, sentences, output_split=output_split), rules))
